...
void event_init(Arena* arena, EventSysConfig config) {
  ...
  st.arena = arena_alloc(arena, config.mem_reserve);
  st.config = config;
}
...
//     if(st.registered[i].events != 0) {
//       // darray_destroy(state.registered[i].events);
//       st.registered[i].events = 0;
//     }
//   }
// }
...
b32 event_register(u32 code, void* listener, PFN_On_Event on_event) {
  ...
  if (st.registered[code].events == null) {
    // st.registered[code].events = push_array(st.arena, RegisteredEvent, 4);
