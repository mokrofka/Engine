...
VK_UIShader vk_ui_shader_create() {
  ...
  String stage_type_strs[UI_ShaderStageCount] = { "vert"_, "frag"_, };
  VkShaderStageFlagBits stage_types[UI_ShaderStageCount] = {VK_SHADER_STAGE_VERTEX_BIT, VK_SHADER_STAGE_FRAGMENT_BIT};

  Loop (i, UI_ShaderStageCount) {
    shader.stages[i] = shader_module_create(BUILTIN_SHADER_NAME_UI, stage_type_strs[i], stage_types[i]);
  }
  ...
  VkDescriptorType descriptor_types[VK_UIShaderDescriptorCount] = {
  ...
  VkDescriptorSetLayoutBinding bindings[VK_UIShaderDescriptorCount] = {};
  Loop (i, VK_UIShaderDescriptorCount) {
  ...
  obj_pool_sizes[0].descriptorCount = VK_MaxUICount ; // how many descriptor in whole you have
  // The second section will be used for image samplers
  obj_pool_sizes[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
  obj_pool_sizes[1].descriptorCount = VK_UIShaderSamplerCount * VK_MaxUICount ;
  ...
  obj_pool_info.maxSets = VK_MaxUICount ;
...
#define ATTRIBUTE_COUNT 2
  ...
  VkPipelineShaderStageCreateInfo stage_create_infos[UI_ShaderStageCount] = {};
  Loop (i, UI_ShaderStageCount) {
  ...
  shader.obj_uniform_buffer = vk_buffer_create(
    sizeof(VK_UIShaderInstUbo) * VK_MaxUICount ,
    VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, 
    true);
...
void vk_ui_shader_apply_material(VK_UIShader* shader, Material* material) {
  if (shader) {
    ...
    VkWriteDescriptorSet descriptor_writes[VK_UIShaderDescriptorCount] = {};
...
void vk_ui_shader_release_resources(VK_UIShader* shader, Material* material) {
  VK_UIShaderInstState* inst_state = &shader->instance_states[material->internal_id];
