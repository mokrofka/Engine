...
Inline b32 char_is_space(u8 c) {
  return c == ' ' || c == '\n' || c == '\t' || c == '\r' || c == '\f' || c == '\v';
}

Inline b32 char_is_upper(u8 c) {
  return 'A' <= c && c <= 'Z';
}

Inline b32 char_is_lower(u8 c) {
  return 'a' <= c && c <= 'z';
}

Inline b32 char_is_alpha(u8 c) {
  return char_is_upper(c) || char_is_lower(c);
}

Inline b32 char_is_slash(u8 c) {
  return c == '/' || c == '\\';
}

Inline u8 char_to_lower(u8 c) {
  if (char_is_upper(c)) {
    c += ('a' - 'A');
  }
  return c;
...
Inline u8 char_to_correct_slash(u8 c) {
  if (char_is_slash(c)) {
    c = '/';
  }
  return c;
