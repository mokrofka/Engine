...
INLINE mat4 operator*(mat4 matrix_0, mat4 matrix_1) {
  ...
  for (i32 row = 0; row < 4; ++row) {
    for (i32 col = 0; col < 4; ++col) {
      result.rows[row].e[col] =
          matrix_0.rows[row].x * matrix_1.rows[0].e[col] +
          matrix_0.rows[row].y * matrix_1.rows[1].e[col] +
          matrix_0.rows[row].z * matrix_1.rows[2].e[col] +
          matrix_0.rows[row].w * matrix_1.rows[3].e[col];
    }
  }
