...
i32 main(i32 args_count, char* args[]) {
	...
	u64 com_slang_modified = os_file_path_properties(com_slang_file_path).modified;
	u64 lib_slang_modified = os_file_path_properties(lib_slang_file_path).modified;
	FileProperties time_stamps_props = os_file_path_properties(time_stamps_file_path);

	struct {
		u64 com_slang_modified;
		u64 lib_slang_modified;
	} stamps;
	b32 is_change = false;
