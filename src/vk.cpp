...
struct GpuGlobalState {
  ...
  // GpuEntity entities[MaxEntities];
...
struct VK_State {
  ...
  mat4 projection;
  u32 debug_lines_max;
  VkDeviceSize debug_lines_offset;
  // Darray<Vertex> debug_lines;
  struct DebugDrawLineTime {
    Vertex vert[2];
    // Vertex vert;
  };
  Darray<f32> debug_line_times;
  Darray<DebugDrawLineTime> debug_lines_remain;
  // Darray<Vertex> debug_lines_remain;
...
intern VkPipeline vk_shader_pipeline_create(ShaderDesc info, VK_ShaderStages stages) {
  // Dynamic rendering
...
Handle<GpuShader> vk_shader_load(String name, ShaderDesc info) {
  Scratch scratch;
  switch (info.type) {
    case ShaderType_Drawing: {
      ...
      shader.pipeline = vk_shader_pipeline_create(info, stages);
...
void vk_shader_reload(String name, Handle<GpuShader> shader_handle, ShaderDesc shader_info) {
  VK_Shader& shader = vk.shaders[shader_handle.handle];
  switch (shader_info.type) {
    case ShaderType_Drawing: {
      ...
      Loop (i, 2) {
        vk.DestroyShaderModule(vkdevice, shader.module[i], vk.allocator);
      }
      VK_ShaderStages stages = vk_shader_module_create(name);
      shader.module[0] = stages.stages[0].module;
      shader.module[1] = stages.stages[1].module;
      shader.pipeline = vk_shader_pipeline_create(shader_info, stages);
    }
    case ShaderType_Screen: {
      // VK_Shader& shader = vk.screen_shader;
      // vk.DeviceWaitIdle(vkdevice);
...
intern void vk_descriptor_init() {
  ...
  // Shader data
  {
    ...
    VkWriteDescriptorSet descriptors[] = {ubo_descriptor, indirect_draw_write_descriptor};
    vk.UpdateDescriptorSets(vkdevice, ArrayCount(descriptors), descriptors, 0, null);
  }
...
Handle<GpuMaterial> vk_material_load(Material material_desc) {
  vk.gpu_materials[vk.gpu_materials_count] = {
    ...
    .specular = material_desc.specular,
    .shininess = material_desc.shininess, 
    .texture = material_desc.texture, 
  };
...
void vk_draw() {
  ...
  if (vk.debug_lines_remain.count > 0) {
    ...
    vk.CmdBindVertexBuffers(cmd, 0, 1, &vk.vert_buffer.handle, &vk.debug_lines_offset);
    vk.CmdDraw(cmd, vk.debug_lines_remain.count, 1, 0, 0);
  }
...
void vk_init() {
  ...
  // Load basic assets
  {
    VK_ShaderStages stages = vk_shader_module_create("line");
    vk.debug_line_shader.module[0] = stages.stages[0].module;
    vk.debug_line_shader.module[1] = stages.stages[1].module;
    ShaderDesc desc = {
    ...
    vk.debug_line_shader.pipeline = vk_shader_pipeline_create(desc, stages);
  }
...
void vk_begin_frame() {
  ...
  if (vk.debug_lines_remain.count > 0) {
    // u32 size = vk.debug_lines.count * sizeof(Vertex);
    // void* data = vk.debug_lines.data;
    u32 size = vk.debug_lines_remain.count * sizeof(Vertex)*2;
    void* data = vk.debug_lines_remain.data;
    u32 offset = vk.vert_buffer.size;
    vk.debug_lines_offset = offset;
    vk_buffer_upload_to_gpu(vk.vert_buffer, {offset, size}, data);
  }
...
void debug_draw_line_time(v3 a, v3 b, v3 color, f32 time) {
  ...
  vk.debug_lines_remain.add({vert[0], vert[1]});
  vk.debug_line_times.add(time);
}
