...
{
    DevWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    DevWindow* backup_hovered_window = g.HoveredWindow;
...
{
    ImGuiContext& g = *GImGui;
    DevWindow* window = g.CurrentWindow;
...
ImGuiID ImGui::GetWindowScrollbarID(DevWindow* window, ImGuiAxis axis)
{
    return window->GetID(axis == ImGuiAxis_X ? "#SCROLLX" : "#SCROLLY");
}

// Return scrollbar rectangle, must only be called for corresponding axis if window->ScrollbarX/Y is set.
ImRect ImGui::GetWindowScrollbarRect(DevWindow* window, ImGuiAxis axis)
{
    ImGuiContext& g = *GImGui;
...
void ImGui::ExtendHitBoxWhenNearViewportEdge(DevWindow* window, ImRect* bb, float threshold, ImGuiAxis axis)
{
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    if (DevWindow* popup_window = FindWindowByName(name))
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    DevWindow* picker_active_window = NULL;
...
static void BoxSelectActivateDrag(ImGuiBoxSelectState* bs, DevWindow* window)
{
    ImGuiContext& g = *GImGui;
...
static void BoxSelectScrollWithMouseDrag(ImGuiBoxSelectState* bs, DevWindow* window, const ImRect& inner_r)
{
    ImGuiContext& g = *GImGui;
...
bool ImGui::BeginBoxSelect(const ImRect& scope_rect, DevWindow* window, ImGuiID box_select_id, ImGuiMultiSelectFlags ms_flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiBoxSelectState* bs = &g.BoxSelectState;
...
static ImRect CalcScopeRect(ImGuiMultiSelectTempData* ms, DevWindow* window)
{
    ImGuiContext& g = *GImGui;
...
void ImGui::EndMenuBar()
{
    ...
    if (NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
        ...
        DevWindow* nav_earliest_child = g.NavWindow;
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p, ImGuiDir dir, float axis_size, ImGuiWindowFlags window_flags)
{
    ...
    DevWindow* bar_window = FindWindowByName(name);
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        DevWindow* child_menu_window = (child_popup && child_popup->Window && child_popup->Window->ParentWindow == window) ? child_popup->Window : NULL;
...
void ImGui::EndMenu()
{
    ...
    DevWindow* parent_window = window->ParentWindow;  // Should always be != NULL is we passed assert.
...
namespace ImGui
{
    ...
    static ImU32            TabBarCalcTabID(ImGuiTabBar* tab_bar, const char* label, DevWindow* docked_window);
...
static ImU32   ImGui::TabBarCalcTabID(ImGuiTabBar* tab_bar, const char* label, DevWindow* docked_window)
{
    ...
    {
        DevWindow* window = GImGui->CurrentWindow;
...
void ImGui::TabBarAddTab(ImGuiTabBar* tab_bar, ImGuiTabItemFlags tab_flags, DevWindow* window)
{
    ImGuiContext& g = *GImGui;
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, DevWindow* docked_window)
{
...
void    ImGui::SetTabItemClosed(const char* label)
{
    ...
    else if (DevWindow* window = FindWindowByName(label))
    {
...
ImVec2 ImGui::TabItemCalcSize(DevWindow* window)
{
